...
const Stats = () => {
  ...
  return (
    <section ref={sectionRef} className="py-20 bg-[#0069B5] text-white relative overflow-hidden">
      {/* Background Pattern */}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-5xl mx-auto">
          {stats.map((stat, index) => {
            ...
            return (
              <div
                key={index}
                className="group text-center p-8 bg-white backdrop-blur-sm rounded-lg border border-white/10 hover:bg-white/10 transition-colors duration-300"
              >
                <div className="inline-flex items-center justify-center w-16 h-16 bg-primary/20 rounded-full mb-4 group-hover:bg-white/20 transition-colors">
                  <Icon className="w-8 h-8 text-primary group-hover:text-white transition-colors" />
                </div>
                <div className="text-5xl font-bold text-primary mb-2 group-hover:text-white transition-colors">
                  {counts[stat.key]}+
                </div>
                <div className="text-lg text-primary/90 group-hover:text-white transition-colors">{stat.label}</div>
              </div>
            );
          })}
        </div>
